...
let controller = {
    ...
    getClients: function(req, res) {
        ...
        //Find paginado
        Client.paginate({}, options, (err, clients) => {
            if (err) {
    ...
    getClientsByUser: function(req, res) {
    ...
    update: function(req, res) {
        ...
        if (validate_name && validate_surname && validate_identification && validate_phone && validate_status && validate_email && validate_address) {
            ...
            Client.findOneAndUpdate({ _id: clientId, user: req.user.sub }, update, { new: true }, (err, clientUpdated) => {
                if (err) {
                    return res.status(500).send({
                        status: 'error',
    ...
    delete: function(req, res) {
        ...
        let clientId = req.params.id;
        //Find and delete por clienteId y por userId
        Client.findOneAndDelete({ _id: clientId, user: req.user.sub }, (err, clientRemoved) => {
            if (err) {
                return res.status(500).send({
                    status: 'error',
                    message: "Error en la petición"
